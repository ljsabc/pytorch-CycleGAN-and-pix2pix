...
def define_D(input_nc, ndf, netD, n_layers_D=3, norm='batch', init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    if netD == 'basic':  # default PatchGAN classifier
        net = NLayerDiscriminatorMulti(input_nc, ndf, n_layers=3, norm_layer=norm_layer)
...
class NLayerDiscriminatorMulti(nn.Module):
    """Defines a PatchGAN discriminator"""
    ...
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d):
        ...
        Parameters:
            ...
            norm_layer      -- normalization layer
        """
        super(NLayerDiscriminatorMulti, self).__init__()
        if type(norm_layer) == functools.partial:  # no need to use bias as BatchNorm2d has affine parameters
            use_bias = norm_layer.func == nn.InstanceNorm2d
        else:
            use_bias = norm_layer == nn.InstanceNorm2d
        ...
        for n in range(n_layers, n_layers+3):  # gradually increase the number of filters
            nf_mult_prev = nf_mult
            nf_mult = min(2 ** n, 8)
            sequence += [
                nn.Conv2d(ndf * nf_mult_prev, ndf * nf_mult, kernel_size=kw, stride=2, padding=padw, bias=use_bias),
